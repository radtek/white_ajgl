...
@Service
@Transactional
public class BailBuilder extends AbstractTaskBuilder implements DbtxComponent {
	
	public static final String BUILDER_TASK_RULE_TYPE = Constant.TASK_TYPE_BAIL + "_Builder";
	
	private static final Logger LOGGER = LoggerFactory.getLogger(BailBuilder.class);
	...
	public boolean builderTask(String businessData) {
		...
		BailData data = (BailData)TaskPojoJSONFmtUtil.getPojo(businessDataObj, BailData.class) ;
		...
		for(String tempTime : timeAt){
			if(!StringUtils.isBlank(tempTime)){
				...
				mapTime.put(cal.getTimeInMillis(), data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，取保候审已经2个月，请安排调查并作笔录");
				//设置4个月底
				cal.add(Calendar.MONTH, 2);
				//暂时不设置月底，否则会导致月底弹窗太多cal.set(Calendar.DAY_OF_MONTH, cal.getActualMaximum(Calendar.DAY_OF_MONTH));
				mapTime.put(cal.getTimeInMillis(), data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，取保候审已经4个月，请安排调查并作笔录");
				//设置开始日期并调整到结束日期
				...
				mapTime.put(cal.getTimeInMillis(), data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，取保候审还有1天期满");
				//设置前7天
				cal.add(Calendar.DAY_OF_MONTH, -6);
				mapTime.put(cal.getTimeInMillis(), data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，取保候审还有7天期满");
				//设置前15天
				cal.add(Calendar.DAY_OF_MONTH, -8);
				mapTime.put(cal.getTimeInMillis(), data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，取保候审还有15天期满");
			}
		}
		
		tg.setName(Constant.TASK_TYPE_BAIL);
