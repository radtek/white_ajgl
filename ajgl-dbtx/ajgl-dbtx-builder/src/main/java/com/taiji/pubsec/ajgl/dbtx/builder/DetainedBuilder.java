...
@Service
@Transactional
public class DetainedBuilder extends AbstractTaskBuilder implements DbtxComponent {

	public static final String BUILDER_TASK_RULE_TYPE = Constant.TASK_TYPE_DETAINED + "_Builder";
	
	private static final Logger LOGGER = LoggerFactory.getLogger(DetainedBuilder.class);
	...
	public boolean builderTask(String businessData) {
		...
		DetainedData data = (DetainedData)TaskPojoJSONFmtUtil.getPojo(businessDataObj, DetainedData.class) ;
		...
		Date sendDate = new Date(data.getApprovalTime());
		for(String tempTime : timeAt){
			if(!StringUtils.isBlank(tempTime)){
				...
				mapTime.put(cal.getTimeInMillis(), "2");
			}
		}
		
		tg.setName(Constant.TASK_TYPE_DETAINED);
		...
		while(iter.hasNext()){
			...
			if(entry.getKey() <= new Date().getTime()){
				...
				DetainedRule rule = new DetainedRule();
				rule.setExecutingTime(mt.getStartTime());
				rule.setContext("" + data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，被刑拘已达" + entry.getValue() + "天，请确认后续工作，如已安排请忽略本消息。");
