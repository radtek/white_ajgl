...
@Service
@Transactional
public class ExtendedDetentionBuilder extends AbstractTaskBuilder implements DbtxComponent {
	
	public static final String BUILDER_TASK_RULE_TYPE = Constant.TASK_TYPE_EXTENDEDDETENTION + "_Builder";
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ExtendedDetentionBuilder.class);
	...
	public boolean builderTask(String businessData) {
		...
		ExtendedDetentionData data = (ExtendedDetentionData)TaskPojoJSONFmtUtil.getPojo(businessDataObj, ExtendedDetentionData.class) ;
		...
		Date sendDate = new Date(data.getExtendedDetentionTime());
		for(String tempTime : timeAt){
			if(!StringUtils.isBlank(tempTime)){
				...
				if(data.getExtendedTime() <= 7){
					cal.add(Calendar.DAY_OF_MONTH, -1);
					mapTime.put(cal.getTimeInMillis(), "");
				}else{
		...
		tg.setName(Constant.TASK_TYPE_EXTENDEDDETENTION);
		...
		while (iter.hasNext()) {
			...
			if(entry.getKey() <= new Date().getTime()){
				...
				ExtendedDetentionRule rule = new ExtendedDetentionRule();
				rule.setExecutingTime(mt.getStartTime());
				rule.setContext("" + data.getPersonName() + data.getCaseCode() + data.getCaseName() + "，被延长拘留已达" + (data.getExtendedTime() > 10 ? data.getExtendedTime() - 7 : data.getExtendedTime() - 1) + "天，请安排后续工作，如已安排请忽略本消息。");
