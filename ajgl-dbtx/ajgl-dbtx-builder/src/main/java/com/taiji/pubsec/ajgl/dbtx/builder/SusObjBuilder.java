...
@Service
@Transactional
public class SusObjBuilder extends AbstractTaskBuilder implements DbtxComponent{
	
	public static final String BUILDER_TASK_RULE_TYPE = Constant.TASK_TYPE_SUSOBJ + "_Builder";
	
	private static final Logger LOGGER = LoggerFactory.getLogger(SusObjBuilder.class);
	...
	public boolean builderTask(String businessData) {
		...
		SusObjData data = (SusObjData)TaskPojoJSONFmtUtil.getPojo(businessDataObj, SusObjData.class) ;
		 ...
		 */
		Long executedTime = data.getSendTime() + 24*60*60*1000 - data.getAheadTime()*60*1000;
		
		tg.setName(Constant.TASK_TYPE_SUSOBJ);
		...
		if(executedTime <= new Date().getTime()){
			...
			SusObjRule rule = new SusObjRule();
			rule.setExecutingTime(mt.getStartTime());
			rule.setAheadTime(data.getAheadTime());
