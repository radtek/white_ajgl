...
public class TemporaryInFragment extends Fragment {

    public static TemporaryInFragment newInstance(String s) {
        TemporaryInFragment newFragment = new TemporaryInFragment();
    ...
    LinearLayout case_in_item_detail_title_lv;
    TextView case_item_time_tv, case_item_number_tv,case_item_name_tv,case_item_person_tv
            ,case_item_file_tv,case_item_title_name_tv,case_item_shelf_tv,case_item_area_tv;
    ...
    CaseTemporaryInStorageData caseTemporaryInStorageData;

    //AppendFileData appendFileData;

    boolean isComit=false;

    public TemporaryInFragment() {
    ...
    {
        if(getActivity().getResources().getString(R.string.isTestTemporaryInAdjustment).equals("1"))
        {
            refreshTestData();
            return;
        }
        CaseTemporaryInStorageData tempData=new CaseTemporaryInStorageData();
            ...
            public void completeBack(String progressId, String result,CaseStorageData caseStorageData) {
                try {
                    if (result.equals("1")) {
                        ...
                        caseTemporaryInStorageData=(CaseTemporaryInStorageData)caseStorageData;
                        if(caseTemporaryInStorageData!=null)
                        {
                            setTextViewText(case_item_title_name_tv,caseTemporaryInStorageData.scanformCode+"暂存物品入库单");
                            setTextViewText(case_item_time_tv,caseTemporaryInStorageData.changeDate(caseTemporaryInStorageData.DataUpdateTime));
                            setTextViewText(case_item_number_tv,caseTemporaryInStorageData.caseCode);
                            setTextViewText(case_item_name_tv,caseTemporaryInStorageData.caseName);
                            setTextViewText(case_item_person_tv,caseTemporaryInStorageData.suspectName);
                            setTextViewText(case_item_file_tv,caseTemporaryInStorageData.papers);
                            setTextViewText(case_item_shelf_tv, caseTemporaryInStorageData.getShelfName());
                            setTextViewText(case_item_area_tv, caseTemporaryInStorageData.areaName);

                            if(case_in_item_detail_title_lv!=null)case_in_item_detail_title_lv.setVisibility(View.VISIBLE);

                            if(caseTemporaryInStorageData.l_caseData!=null && caseTemporaryInStorageData.l_caseData.size()>0)
                            {
                                String tempStr= caseTemporaryInStorageData.getShelfName();
                                //入库了隐藏下一步
                                ...
                                caseTemporaryInStorageData.sortCaseData(caseTemporaryInStorageData.l_caseData);
                                if(caseTemporaryAdapter!=null)
                                {
                                    caseTemporaryAdapter.setDataSource(caseTemporaryInStorageData.getList_caseData());
                        ...
                        setTextViewText(case_item_title_name_tv, "暂存物品入库单信息加载失败");
                        mListView.removeFooterView(loadingView);
                        mListView.addFooterView(noDataView);
                        if(btn_case_item_btn_next!=null) {
                        ...
                        if(case_in_item_detail_title_lv!=null)case_in_item_detail_title_lv.setVisibility(View.GONE);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fg_case_item_temporary_in, container, false);
        mView=view;
...
*/
        ...
        btn_case_item_btn_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mBundle.putSerializable("showType", 7);//1入 2出 3返 4调整出 5调整入  7暂入　８暂出
                if (caseTemporaryInStorageData != null)
                    mBundle.putSerializable("CaseStorageData", caseTemporaryInStorageData);
        ...
        setTextViewText(case_item_area_tv,"");
         ...
         setTextViewText(case_item_title_name_tv,"正在加载暂存物品入库单信息...");
    ...
    private class FetchItemsTask extends AsyncTask<Integer, Void, CaseTemporaryInStorageData> {

        @Override
        protected CaseTemporaryInStorageData doInBackground(Integer... params) {
        ...
        protected void onPostExecute(CaseTemporaryInStorageData caseStorageData) {
            mListView.removeFooterView(loadingView);
            fetchItemsTaskRun=false;
            caseTemporaryInStorageData =caseStorageData;
            caseTemporaryAdapter.addMoreItems(caseTemporaryInStorageData.getList_caseData());
            setTextViewText(case_item_title_name_tv, caseTemporaryInStorageData.scanformCode + "暂存物品入库单");
            setTextViewText(case_item_time_tv, caseTemporaryInStorageData.changeDate(caseTemporaryInStorageData.DataUpdateTime));
            setTextViewText(case_item_number_tv,caseTemporaryInStorageData.caseCode);
            setTextViewText(case_item_name_tv,caseTemporaryInStorageData.caseName);
