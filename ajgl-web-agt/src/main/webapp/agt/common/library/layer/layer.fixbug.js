...
Class.pt.vessel = function(conType, callback){
    ...
    var marginSetStr = (((config.type===1)&&config.marginSet!=undefined)?' style="padding:'+config.marginSet+'px;"' : '') ;
    callback([
        //遮罩
        ...
        '<div class="'+ doms[0] +' '+ (doms.anim[config.shift]||'') + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
            ...
            +'<div class="layui-layer-content'+ ((config.type == 0 && config.icon !== -1) ? ' layui-layer-padding' :'') + (config.type == 3 ? ' layui-layer-loading'+config.icon : '') +'"'+ marginSetStr +'>'
    ...
    switch(config.type){
        ...
        case 2:
            ...
            if(config.pageLoading){
            	loadClass = "layui-layer-load" ;
            }else{
            	loadClass = "layui-layer-noload" ;
            }
            config.content = '<iframe scrolling="'+ (config.content[1]||'auto') +'" allowtransparency="true" id="'+ doms[4] +''+ times +'" name="'+ doms[4] +''+ times +'" onload="this.className=\'\';" class="'+loadClass+'" frameborder="0" src="' + config.content[0] + '"></iframe>';
        break;
        case 3:
            config.title = false;
...
Class.pt.callback = function(){
    ...
    layero.find('.'+ doms[6]).children('a').on('click', function(){
        ...
        if(index === 0){
        	if(config.btn1!=undefined){
        		config['btn'+(index+1)] ? config['btn'+(index+1)](that.index, layero) : layer.close(that.index);
        	}else{
        		config.yes ? customYes(that.index, layero) : layer.close(that.index);
        	}      
        } else if(index === 1){
        	if(config.btn2!=undefined){
        		config['btn'+(index+1)] ? config['btn'+(index+1)](that.index, layero) : layer.close(that.index);
        	}else{
        		cancel();
        	}    
    ...
    //确定
    function customYes(index, layero){
        var yes = config.yes && config.yes(index, layero);
        yes === false || layer.close(index);
    }
...
'function' === typeof define ? define(function(){
   ...
   ready.run();
   layer.use('skin/layer.fixbug.css');
}();
