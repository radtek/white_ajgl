(function($){
	...
	$(document).ready(function() {	
		...
		$(document).on("click","#newLocker",function(){
			if($.util.isBlank(code)){
				window.top.$.layerAlert.alert({msg:"请先新建保管区！"}) ;
				return;
			}
			window.top.$.layerAlert.dialog({
				content : context +  '/lockerMaintain/showNewLockerListPage.action',
				pageLoading : true,
				title:"新增储物箱",
				width : "500px",
				height : "600px",
		...
		$(document).on("click","#modify",function(){
			...
			var row=lockerTable.row(tr);	//行对象
			var data=row.data();	//行所有数据
			
			window.top.$.layerAlert.dialog({
				content : context +  '/lockerMaintain/showModifyLockerListPage.action',
				pageLoading : true,
				title:"修改储物箱",
				width : "500px",
				height : "600px",
		...
		$(document).on("click","#delete",function(){
			...
			window.top.$.layerAlert.confirm({
				...
				yes:function(index, layero){
					//点击确定按钮后执行
					$.ajax({
						url:context +'/lockerMaintain/deleteLocker.action',
						type:'post',
						dataType:'json',
						data:dataMap,
	...
	function initStorageArea(){
		$.ajax({
			url:context +'/storageArea/findStorageAreaByFrame.action',
			type:'post',
			dataType:'json',
				 ...
				 $($(row).children("td")).each(function(i, val){
						if(i!=0){
							$(val).on("click", function(){		
								...
								var id=data.id;
								window.top.$.layerAlert.dialog({
									content : context +  '/lockerMaintain/showLookLockerListPage.action',
									pageLoading : true,
									title:"查看储物箱",
									width : "450px",
									height : "470px",
