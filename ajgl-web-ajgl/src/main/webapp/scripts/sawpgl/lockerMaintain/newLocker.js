(function($){
	...
	$(document).ready(function() {	
		$.when(
				$.ajax({
					url:context +'/lockerMaintain/generateCode.action',
					type:'post',
					dataType:'json',
...
//		    	$.select2.able("#storageArea",false);
	...
	function saveLocker(){
		...
		dataMap["articleLockerBean.code"] = $("#jCode").val()+"-"+$("#xCode").val();
...
//		dataMap["articleLockerBean.area.id"] = $("#storageArea").select2("val");    //    取值
		...
		var arr = $.icheck.getChecked("lockerStatus");
		dataMap["articleLockerBean.status"] =arr.length == 1 ? $.common.Constant.SF_S() : $.common.Constant.SF_F();
		$.ajax({
			url:context +'/lockerMaintain/newLocker.action',
			type:'post',
			dataType:'json',
			data:dataMap,
			success:function(json){
