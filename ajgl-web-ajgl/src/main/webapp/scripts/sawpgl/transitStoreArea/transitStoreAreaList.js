(function($){
	...
	var transitStoreAreaTable = null;//暂存物品保管区table
	
	$(document).ready(function() {	
		initTransitStoreAreaTable();
		/**
		 * 新增按钮点击事件
		 */
		$(document).on("click","#saNew",function(){
			window.top.$.layerAlert.dialog({
				content : context +  '/transitStoreArea/showNewOrUpdateTransitStoreAreaPage.action',
				pageLoading : true,
				title:"新增暂存物品保管区",
				width : "500px",
				height : "400px",
				...
				initData:{
					...
					transitStoreAreaTable.draw(false);
				}
			});
		});
		/**
	...
	function updateSaStatus(saIdList, state, msg){
		...
		$.ajax({
			url:context +'/transitStoreArea/updateTransitStoreAreaState.action',
			type:'post',
			dataType:'json',
			data:gData,
	...
	function deleteSaByIds(saIdList){
		...
		$.ajax({
			url:context +'/transitStoreArea/deleteTransitStoreAreaByIds.action',
			type:'post',
			dataType:'json',
			data:gData,
			success:function(successData){
	 ...
	 * 初始化暂存物品保管区table
	 */
	function initTransitStoreAreaTable(){
		var tb = $.uiSettings.getOTableSettings();
		tb.ajax.url = context + "/transitStoreArea/findtransitStoreAreaByPaging.action";
		tb.columnDefs = [
			{
				"targets": 0,
     	    	"width": "50px",
		...
		transitStoreAreaTable = $("#transitStoreAreaTable").DataTable(tb);
	}
