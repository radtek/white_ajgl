    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            ...
            if(arr[1]!=undefined && arr[1] < 1){
                arr[1] = 1;
                isvoid.auto = 1;
            } else if(arr[1]!=undefined && arr[1] > 12){
                arr[1] = 12;
                isvoid.auto = 1;
            } else if(arr[1]!=undefined && arr[1].length < 2){
                isvoid.auto = 1;
            }
            if(arr[2]!=undefined && arr[2] < 1){
                arr[2] = 1;
                isvoid.auto = 1;
            } else if(arr[2]!=undefined && arr[2] > Dates.months[(arr[1]|0)-1]){
                arr[2] = 31;
                isvoid.auto = 1;
            } else if(arr[2]!=undefined && arr[2].length < 2){
                isvoid.auto = 1;
            }
            if(arr.length > 3){
...
Dates.iswrite = function(){
    var S = Dates.query, log = {
        time: S('#laydate_hms'),
        hour: S('#laydate-hour'),
        minute: S('#laydate-minute'),
        second: S('#laydate-second'),
        hourSpan: S('#laydate-hour-span'),
        minuteSpan: S('#laydate-minute-span')
    };
    ...
    var fmt = Dates.options.format ;
    if(fmt.indexOf("hh")<0){
    	Dates.shde(log.hour, true);
    }else{
    	Dates.shde(log.hour, false);
    }
    if(fmt.indexOf("mm")<0){
    	Dates.shde(log.minute, true);
    }else{
    	Dates.shde(log.minute, false);
    }
    if(fmt.indexOf("ss")<0){
    	Dates.shde(log.second, true);
    }else{
    	Dates.shde(log.second, false);
    }
    
    if(fmt.indexOf("mm")<0){
    	Dates.shde(log.hourSpan, true);
    }else{
    	Dates.shde(log.hourSpan, false);
    }
    
    if(fmt.indexOf("ss")<0){
    	Dates.shde(log.minuteSpan, true);
    }else{
    	Dates.shde(log.minuteSpan, false);
    }
};

//方位辨别
Dates.orien = function(obj, pos, elem, options){
    var tops, rect = Dates.elem.getBoundingClientRect();
    var datePos = $(elem).attr("date-pos") ;
    obj.style.left = rect.left + (pos ? 0 : Dates.scroll(1)) + 'px';

    if(datePos==="top"){
    	tops = rect.top - obj.offsetHeight + 1 ;
    	//tops = rect.top > obj.offsetHeight/1.5 ? rect.top - obj.offsetHeight + 1 : Dates.winarea() - obj.offsetHeight; 
//    	if(rect.top + obj.offsetHeight/1.5 <= Dates.winarea()){
//    		tops = rect.top - obj.offsetHeight + 1 ;  
//    	}else{
//    		tops = rect.bottom > obj.offsetHeight/1.5 ? rect.bottom - 1 : Dates.winarea() - obj.offsetHeight;  
//    	}   	
    }else{
...
//吸附定位
Dates.follow = function(obj, elem, options){
    if(Dates.options.fixed){
        obj.style.position = 'fixed';
        Dates.orien(obj, 1, elem, options);
    } else {
        obj.style.position = 'absolute';
        Dates.orien(obj, undefined, elem, options);
    }
};
...
//                if(fmt.indexOf("hh")>0){
//                	if(fmt.indexOf("mm")>0){
//                		return '<li><input readonly>:</li>' ;
//                	}else{
//                		return '<li><input readonly></li>' ;
//                	}          	
//                }else{
//                	return '<li><input readonly style="display:none"></li>' ;
//                }
//            }()+function(){
//                /******************************时分秒是否显示********************************/
//                var fmt = options.format ;
...
//                }
//            }()
            +'<li id="laydate-hour"><input readonly><span id="laydate-hour-span" style="float:left">:</span></li>'
            +'<li id="laydate-minute"><input readonly><span id="laydate-minute-span" style="float:left">:</span></li>'
            +'<li id="laydate-second"><input readonly></li>'
          +'</ul>'
    ...
    Dates.follow(Dates.box, elem, options);
...
Dates.events = function(){
    ...
    log.tabMonth = function(type){
        ...
        if(Dates.ymd[1]===1 && Dates.ymd[2]>28){
        	var ymddtp = Dates.ymd[2] ;
        	Dates.viewDate(Dates.ymd[0], Dates.ymd[1], 28);
        	Dates.ymd[2] = ymddtp ;
        }else{
        	Dates.viewDate(Dates.ymd[0], Dates.ymd[1], Dates.ymd[2]);
        }
...
laydate.reset = function(){
    (Dates.box && Dates.elem) && Dates.follow(Dates.box, Dates.elem, Dates.options);
};
